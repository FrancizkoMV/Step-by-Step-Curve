...
function renderInputs() {
  ...
  if (totalWidth <= 0) {
    container.innerHTML = '<div style="padding: 12px; color: #64748b; text-align: center; font-size: 0.82rem;">Mide e ingresa el <b>Ancho Total</b> arriba para desplegar las casillas de medición.</div>';
    return;
  }
  ...
  for (let i = 0; i <= centerIndex; i++) {
    ...
    let badgeHTML = '';
    if (isFirst) badgeHTML = `<div class="badge">🏁 Inicio (X=0.00m)</div>`;
    else if (isCenter) badgeHTML = `<div class="badge">📌 Centro (${p.x.toFixed(2)}m)</div>`;

    row.innerHTML = `
      ${badgeHTML}
      <div class="point-field">
        <label>X (${i}) [m]</label>
        <input type="text" value="${p.x.toFixed(2)}" disabled style="background-color: #e2e8f0; color: #475569;">
      </div>
      <div class="point-field">
        <label>Y (${i}) [m]</label>
        <input type="number" step="0.01" min="0" max="9999.99" inputmode="decimal" placeholder="0.00" value="${currentY}" ${isCenter ? 'disabled style="background-color: #fef08a; color: #854d0e;"' : ''} oninput="updatePointY(${i}, this.value)">
      </div>
    `;
...
function exportPDF() {
  ...
  doc.addImage(canvasHalf.toDataURL('image/png', 1.0), 'PNG', 14, 117, 182, 70);
